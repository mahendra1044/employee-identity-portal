...
function copyToClipboard(data: any, name?: string, successMessage?: string) {
  navigator.clipboard.writeText(JSON.stringify(data, null, 2));
  toast.success(successMessage || `Copied ${name || 'data'} to clipboard`);
}
...
function TableView({ data, noDataText }: { data: unknown[]; noDataText?: string }) {
  if (!Array.isArray(data) || data.length === 0) {
    return <p className="text-xs text-muted-foreground">{noDataText || 'No data available'}</p>;
  }
